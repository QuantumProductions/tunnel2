...
takable(Board, X, {Rank, File}) ->
  takable([{tile(Board, {Rank - 1, File}), west},
...
takable([], _) ->
  false;
takable([{null, _WallDirection} | T], X) ->
  takable(T, X);
takable([{{_, null, _}, _} | T], X) ->
  takable(T, X);
takable([{{_Status, TilePlayer, WallDirection}, WallDirection} | T], X) when TilePlayer =/= X ->
  takable(T, X);
takable(_, _) ->
  true.  
...
wall(Board, WallDirection, {Rank, File}) ->
  ...
  replace(Board, {Status, Player, WallDirection}, Rank, File).
